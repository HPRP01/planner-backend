...
WORKDIR /opt/app
COPY --from=maven /usr/src/app/target/planner-application.jar /opt/app/

EXPOSE 8080
ENTRYPOINT ["java", "-jar", "planner-application.jar"]
...
# VOLUME /tmp
# COPY target/*.jar PlannerApplication.jar
...
# ENTRYPOINT ["java", "-jar","/PlannerApplication.jar"]